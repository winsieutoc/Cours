...
template <typename _PointType, typename _DataRecord> struct GenericKDTreeNode {
    ...
    void setLeftIndex(IndexType self, IndexType value) {
        if (value != self+1)
            throw RTException("GenericKDTreeNode::setLeftIndex(): Internal error!");
    }
...
public:
    ...
    typedef Eigen::AlignedBox<Scalar,PointType::Dimension> BoundingBoxType;
...
public:
    ...
    void clear() { m_nodes.clear(); m_bbox.setNull(); }
    ...
    void push_back(const NodeType &node) {
        ...
        m_bbox.extend(node.getPosition());
    }
    ...
    void build(bool recomputeBoundingBox = false) {
        ...
        if (recomputeBoundingBox) {
            ...
            for (size_t i=0; i<m_nodes.size(); ++i)
                m_bbox.extend(m_nodes[i].getPosition());
        }
...
protected:
    ...
    void build(size_t depth,
        ...
        if (rangeEnd <= rangeStart)
            throw RTException("Internal error!");
        ...
        switch (m_heuristic) {
            case Balanced: {
                    ...
                    (m_bbox.max() - m_bbox.min()).maxCoeff(&axis);
                };
                break;
