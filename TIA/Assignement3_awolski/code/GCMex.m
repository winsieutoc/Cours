...
%     of the N nodes in the graph (N nombre de pixel)
%   UNARY:: A CxN matrix specifying the potentials (data term) for
%     each of the C possible classes at each of the N nodes. (C valeur de
%     l'energie)
%   PAIRWISE:: An NxN sparse matrix specifying the graph structure and
%     cost for each link between nodes in the graph.
%   LABELCOST (Delta xp-xq):: A CxC matrix specifying the fixed label cost for the
%     labels of each adjacent node in the graph.
%   EXPANSION (osef):: A 0-1 flag which determines if the swap or expansion
